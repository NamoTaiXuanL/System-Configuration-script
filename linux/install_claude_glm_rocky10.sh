...
# Claude+GLM配置脚本 for Rocky Linux 10
...
detect_system() {
    ...
    if [[ "$ID" != "rocky" && "$ID_LIKE" != *"rhel"* && "$ID_LIKE" != *"fedora"* ]]; then
        log_warning "此脚本专为Rocky Linux设计，当前系统: $NAME"
        read -p "是否继续？(y/N): " -n 1 -r
        echo
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then
            exit 0
...
enable_repositories() {
    ...
    if ! dnf repolist enabled | grep -q "epel"; then
        ...
        log_success "EPEL仓库已启用"
    else
        log_info "EPEL仓库已启用"
    fi

    # 启用PowerTools (或 crb) 仓库
    if dnf repolist all | grep -q "power-tools"; then
        sudo dnf config-manager --set-enabled power-tools
        log_success "PowerTools仓库已启用"
    elif dnf repolist all | grep -q "crb"; then
        sudo dnf config-manager --set-enabled crb
        log_success "CRB仓库已启用"
    else
        log_info "未找到PowerTools/CRB仓库，继续安装..."
    fi
...
# 使用NodeSource安装最新版Node.js（备选方案）
install_node_nodesource() {
    log_info "使用NodeSource安装最新版Node.js..."
...
show_usage() {
    ...
    echo "  --nodesource    使用NodeSource安装最新版Node.js"
}

# 主函数
main() {
    local mode="install"
    local use_nodesource=false
